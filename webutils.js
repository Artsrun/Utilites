...
window.TWidgetAuth = {
  ...
  logIn: function() {
    ...
    Tel.Login.auth({bot_id: options.bot_id, lang: 'en'}, function(user) {
      if (user) {
...
(function() {
    ...
    init: function(playerEl) {
      ...
      if (inGroup) {
        addEvent(playerEl, 'click', function videoToggleInGroup(e) {
          if (e.metaKey || e.ctrlKey) return true;
          if (hasClass(e.target, 'message_media_view_in_tel')) return true;
          e.stopPropagation();
  ...
  var TWidgetLogin = {
    ...
    auth: function() {
      ...
      TWidgetLogin.activePopup = window.open('/auth?bot_id=' + TWidgetLogin.botId + (TWidgetLogin.paramsEncoded ? '&' + TWidgetLogin.paramsEncoded : ''), 'tel_oauth', 'width=' + width + ',height=' + height + ',left=' + left + ',top=' + top + ',status=0,location=0,menubar=0,toolbar=0');
